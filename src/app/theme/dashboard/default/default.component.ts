...
totaldays:any;
  ...
  ngOnInit() {
debugger
    let qry = "SELECT sum(TotalAmount) total from t_sale_master where YEAR(CreatedDate) = YEAR(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry}).subscribe(item=>{
     this.totalsales = item["data"][0].total
    
    })
        let qry1 = "SELECT sum(TotalAmount) totalmonth from t_sale_master where month(CreatedDate) = month(CURDATE())"
   this.api.Post("/users/executeSelectStatement",{Query : qry1}).subscribe(item=>{
     this.totalmonth = item["data"][0].totalmonth
    
    })
    let qry2 = "SELECT sum(TotalAmount) totalmonth from t_sale_master where (CreatedDate) = (CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry2}).subscribe(item=>{
      this.totaldays = item["data"][0].totalmonth
